...
When doing multi-process development, MSBuild.exe's can build up if a process is stopped at the wrong time. So here's a script to kill all those "other" processes that keep references to files, sockets and VirtualAlloc's.
...
== SystemStability ==
Registry settings and scripts to help test 24/7 system stability requirements.
