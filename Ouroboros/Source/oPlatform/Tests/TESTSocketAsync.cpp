...
struct PLATFORM_oSocketAsync : public oTest
{
	...
	RESULT TestTCPServerASYNC(char* _StrStatus, size_t _SizeofStrStatus)
	{
		...
		Receiver->ExpectedMessageLength = as_uint(strlen(TESTSocketTCP0)) + 1;
	...
	RESULT Run(char* _StrStatus, size_t _SizeofStrStatus) override
	{
		...
		ouro::task_group* g = ouro::new_task_group();
		ouro::finally onexit([&] { g->wait(); ouro::delete_task_group(g); });
