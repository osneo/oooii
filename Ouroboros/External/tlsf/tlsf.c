...
// @oooii-tony: int -> size_t to quiet compiler warnings
...
#ifdef _M_X64
	FL_INDEX_MAX = 60, // @oooii-tony: Bump this up for 64-bit
#else
	FL_INDEX_MAX = 30,
#endif
...
//tlsf_static_assert(sizeof(size_t) * CHAR_BIT == 32); // @oooii-tony: Testing 64-bit issues

/* SL_INDEX_COUNT must be <= number of bits in sl_bitmap's storage type. */
tlsf_static_assert(sizeof(size_t) * CHAR_BIT >= SL_INDEX_COUNT);

/* sizeof fl_bitmap must be >= FL_INDEX_COUNT. */
tlsf_static_assert(sizeof(size_t) * CHAR_BIT >= FL_INDEX_COUNT);
...
static const size_t block_size_max = 1LL << FL_INDEX_MAX;
...
static size_t block_is_free(const block_header_t* block)
{
...
static size_t block_is_prev_free(const block_header_t* block)
{
...
static void mapping_insert(size_t size, size_t* fli, size_t* sli)
{
	size_t fl, sl;
...
static void mapping_search(size_t size, size_t* fli, size_t* sli)
{
...
static block_header_t* search_suitable_block(pool_t* pool, size_t* fli, size_t* sli)
{
	size_t fl = *fli;
	size_t sl = *sli;
	...
	** fl/sl index.
	*/
	unsigned int sl_map = pool->sl_bitmap[fl] & (WORD_MASK << sl);
	if (!sl_map)
	{
		/* No block exists. Search in the next largest first-level list. */
		const unsigned int fl_map = pool->fl_bitmap & (WORD_MASK << (fl + 1));
		if (!fl_map)
		{
...
static void remove_free_block(pool_t* pool, block_header_t* block, size_t fl, size_t sl)
{
...
static void insert_free_block(pool_t* pool, block_header_t* block, size_t fl, size_t sl)
{
