...
HRESULT CGPUBC6HEncoder::GPU_BC6HEncodeAndSave( ID3D11Texture2D* pSourceTexture,
                                                DXGI_FORMAT fmtEncode, WCHAR* strDstFilename, ID3D11Texture2D** ppOutCompressedTexture ) // @oooii-tony: Add an output texture (rather than saving a file)
{
    HRESULT hr = S_OK;
    ...
    V_RETURN( GPU_SaveToFile( m_pDevice, m_pContext,
                              ...
                              fmtEncode, m_pEncodedTextureAsBuf, ppOutCompressedTexture ) ); // @oooii-tony: Add an output texture (rather than saving a file)
    printf( "done\n" );
...
HRESULT CGPUBC6HEncoder::GPU_SaveToFile( ID3D11Device* pDevice, ID3D11DeviceContext* pContext,
                                         ID3D11Texture2D* pSrcTexture,
                                         WCHAR* strFilename,
                                         DXGI_FORMAT dstFormat, ID3D11Buffer* pTextureAsBuf, ID3D11Texture2D** ppOutCompressedTexture ) // @oooii-tony: Add an output texture (rather than saving a file)
{
    HRESULT hr = S_OK;
...
HRESULT CGPUBC6HDecoder::GPU_BC6HDecodeAndSave( ID3D11Texture2D* pSourceTexture, WCHAR* strDstFilename, ID3D11Texture2D** ppOutCompressedTexture ) // @oooii-tony: Add an output texture (rather than saving a file)
{
    HRESULT hr = S_OK;
    ...
    pSrcTexDevice->GetImmediateContext( &pSrcTexContext ); pSrcTexContext->Release();
    {
        ...
        sbInputDesc.ByteWidth = texSrcDesc.Height * texSrcDesc.Width * sizeof( BufferBC6H ) / BLOCK_SIZE;
    ...
    hr = GPU_BC6HDecodeAndSaveInternal( pBufferInput, texSrcDesc.Width, texSrcDesc.Height, texSrcDesc.Format, strDstFilename, ppOutCompressedTexture ); // @oooii-tony: Add an output texture (rather than saving a file)
...
HRESULT CGPUBC6HDecoder::GPU_BC6HDecodeAndSaveInternal( ID3D11Buffer* pSrcTextureAsBuf, UINT uWidth, UINT uHeight, DXGI_FORMAT fmtSrc, WCHAR* strDstFilename, ID3D11Texture2D** ppOutCompressedTexture ) // @oooii-tony: Add an output texture (rather than saving a file)
{
    ID3D11ShaderResourceView* pSRV = NULL;
    ...
    D3D11_BUFFER_DESC sbOutDesc;
    {
        ...
        sbOutDesc.ByteWidth = uWidth * uHeight * sizeof( BufferR32G32G32A32UINT );
    ...
    //if ( strDstFilename ) // @oooii-tony: Add an output texture (rather than saving a file)
        //hr = D3DX11SaveTextureToFile( m_pContext, m_pRestoredTexture, D3DX11_IFF_DDS, strDstFilename ); // @oooii-tony: Add an output texture (rather than saving a file)
