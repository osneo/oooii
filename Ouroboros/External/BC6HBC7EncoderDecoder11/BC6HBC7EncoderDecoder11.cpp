...
// @oooii-tony: Convert all these c-style + global functions to a single class
// so we can more properly support threadlocal execution.
class BC6HBC7EncoderDecoder11_Sample
{
public:
...
BC6HBC7EncoderDecoder11_Sample(ID3D11Texture2D* _pSourceTexture)
	: g_pDevice(NULL)
	, g_pContext(NULL)
	, g_pSourceTexture(_pSourceTexture)
	, g_bCS4xAvailable(FALSE)
{
	if (g_pSourceTexture)
		g_pSourceTexture->AddRef();
}

~BC6HBC7EncoderDecoder11_Sample()
{
	if (g_pSourceTexture)
		g_pSourceTexture->Release();
}

// @oooii-tony: No need for forward declaration in a class
...
BOOL ParseCommandLine( int argc, WCHAR* argv[] )
{
    ...
    for ( int i = 1; i < argc; ++i )
    {
        ...
        if ( argv[i][0] == L'/' )
        {
            //wprintf( L"Unknown option %s\n", argv[i] ); // @oooii-tony: Squelch spew
            return FALSE;
        }
    }
...
int __cdecl wmain(int argc, WCHAR* argv[], ID3D11Texture2D** ppOutCompressedTexture) // @oooii-tony: Add an output texture (rather than saving a file)
{
...
You can choose to save this reconstructed texture using /srf\n\n" );
    ...
    // Create the hardware device with the highest possible feature level
		// @oooii-tony: Always use the device from the source texture
#if 0 // @oooii-tony
    ...
    printf( "done\n" );
#else // @oooii-tony
		g_pSourceTexture->GetDevice(&g_pDevice); // @oooii-tony
		g_pDevice->GetImmediateContext(&g_pContext); // @oooii-tony
#endif // @oooii-tony
    ...
    for ( int i = 1; i < argc; ++i )
    {
        ...
        if ( argv[i][0] == L'/' )
            continue;
#if 0 // @oooii-tony: Don't load source texture from file, it has been specified in the ctor
        if ( !FileExists( argv[i] ) )
        {
            wprintf( L"\nFile not found: %s\n", argv[i] );
            continue;
        }       
#endif // @oooii-tony: Don't load source texture from file, it has been specified in the ctor
        ...
        g_pSourceTexture->GetDesc( &srcTexDesc );
				...
				// @oooii-tony: Do a conversion if necessary
				{
					if (fmtLoadAs != DXGI_FORMAT_FROM_FILE && srcTexDesc.Format != fmtLoadAs)
					{
						D3D11_TEXTURE2D_DESC newDesc = srcTexDesc;
						newDesc.Format = fmtLoadAs;
						ID3D11Texture2D* pNewTexture = NULL;
						if ( FAILED( g_pDevice->CreateTexture2D( &newDesc, NULL, &pNewTexture ) ) )
							continue;

						if ( FAILED( D3DX11LoadTextureFromTexture( g_pContext, g_pSourceTexture, NULL, pNewTexture ) ) )
							continue;

						// spoof source texture with this new compatible one
						g_pSourceTexture->Release();
						g_pSourceTexture = pNewTexture;
					}
				}
        ...
        {
            // Encode to BC7
            if ( FAILED( EncodeDecodeBC7( argv[i], g_pSourceTexture, ppOutCompressedTexture ) ) ) // @oooii-tony: Add an output texture (rather than saving a file)
                continue;            
        } else
        if ( g_CommandLineOptions.mode == CCommandLineOptions::MODE_ENCODE_BC6HS || 
             g_CommandLineOptions.mode == CCommandLineOptions::MODE_ENCODE_BC6HU )
        {
            // Encode to BC6H
            if ( FAILED( EncodeDecodeBC6H( argv[i], g_pSourceTexture, ppOutCompressedTexture ) ) ) // @oooii-tony: Add an output texture (rather than saving a file)
                continue;            
        } else
        if ( g_CommandLineOptions.mode == CCommandLineOptions::MODE_DECODE )
        {
            if ( srcTexDesc.Format == DXGI_FORMAT_BC7_UNORM )
            {
                ...
                if ( FAILED( DecodeBC7( argv[i], g_pSourceTexture, ppOutCompressedTexture ) ) ) // @oooii-tony: Add an output texture (rather than saving a file)
                    continue;
            } else
            if ( srcTexDesc.Format == DXGI_FORMAT_BC6H_SF16 || srcTexDesc.Format == DXGI_FORMAT_BC6H_UF16 )
            {
                ...
                if ( FAILED( DecodeBC6H( argv[i], g_pSourceTexture, ppOutCompressedTexture ) ) ) // @oooii-tony: Add an output texture (rather than saving a file)
                    continue;                  
            } else
            {
...
HRESULT EncodeDecodeBC7( WCHAR* strSrcFilename, ID3D11Texture2D* pSourceTexture, ID3D11Texture2D** ppOutCompressedTexture ) // @oooii-tony: Don't write to file, just pass out the texture
{
    HRESULT hr = S_OK;
    ...
    if ( g_bCS4xAvailable )
    {
        ...
        V_RETURN( g_GPUBC7Encoder.GPU_BC7EncodeAndSave( pSourceTexture, DXGI_FORMAT_BC7_UNORM, &fname[0], ppOutCompressedTexture ) ); // @oooii-tony: Add an output texture (rather than saving a file)
        ...
        if ( g_CommandLineOptions.bSaveReconstructionFile || g_CommandLineOptions.bShowReconstructionError )
        {
            ...
            V_RETURN( g_GPUBC7Decoder.GPU_BC7DecodeAndSaveInternal( g_GPUBC7Encoder.GetEncodedTextureAsBuf(), srcTexDesc.Width, srcTexDesc.Height, DXGI_FORMAT_BC7_UNORM, 
                                                                    g_CommandLineOptions.bSaveReconstructionFile ? &fname[0] : NULL, ppOutCompressedTexture ) );
    ...
    } else
    {
        ...
        //wprintf( L"\tSaving to %s...", &fname[0] ); // @oooii-tony: Squelch spew
...
HRESULT EncodeDecodeBC6H( WCHAR* strSrcFilename, ID3D11Texture2D* pSourceTexture, ID3D11Texture2D** ppOutCompressedTexture ) // @oooii-tony: Add an output texture (rather than saving a file)
{
    HRESULT hr = S_OK;
    ...
    if ( g_bCS4xAvailable )
    {
        ...
        V_RETURN( g_GPUBC6HEncoder.GPU_BC6HEncodeAndSave( pSourceTexture, 
            g_CommandLineOptions.mode == CCommandLineOptions::MODE_ENCODE_BC6HS ? DXGI_FORMAT_BC6H_SF16 : DXGI_FORMAT_BC6H_UF16, &fname[0], ppOutCompressedTexture ) ); // @oooii-tony: Add an output texture (rather than saving a file)
...
HRESULT DecodeBC7( WCHAR* strSrcFilename, ID3D11Texture2D* pSourceTexture, ID3D11Texture2D** ppUncompressedTexture ) // @oooii-tony: Add an output texture (rather than saving a file)
{
    HRESULT hr = S_OK;        
    ...
    if ( g_bCS4xAvailable )
    {
        V_RETURN( g_GPUBC7Decoder.GPU_BC7DecodeAndSave( pSourceTexture, &fname[0], ppUncompressedTexture ) ); // @oooii-tony: Add an output texture (rather than saving a file)
    } else
    {
        ID3D11Texture2D* pTexRestored = NULL; // @oooii-tony: Add an output texture (rather than saving a file)
        printf( "\tDecoding from BC7..." );
        V_RETURN( CPU_EncodeDecode( g_pDevice, g_pContext, pSourceTexture, DXGI_FORMAT_R8G8B8A8_UNORM, ppUncompressedTexture ) ); // @oooii-tony: Add an output texture (rather than saving a file)
        printf( "done\n" );

        //wprintf( L"\tSaving to %s...", &fname[0] ); // @oooii-tony: Squelch spew
        //D3DX11SaveTextureToFile( g_pContext, pTexRestored, D3DX11_IFF_DDS, &fname[0] ); // @oooii-tony: Add an output texture (rather than saving a file)
        printf( "done\n" );
        //SAFE_RELEASE( pTexRestored ); // @oooii-tony: Add an output texture (rather than saving a file)
    }

    return hr;
}

HRESULT DecodeBC6H( WCHAR* strSrcFilename, ID3D11Texture2D* pSourceTexture, ID3D11Texture2D** ppUncompressedTexture ) // @oooii-tony: Add an output texture (rather than saving a file)
{
    HRESULT hr = S_OK;
    ...
    if ( g_bCS4xAvailable )
    {
        V_RETURN( g_GPUBC6HDecoder.GPU_BC6HDecodeAndSave( pSourceTexture, &fname[0], ppUncompressedTexture ) ); // @oooii-tony: Add an output texture (rather than saving a file)
    } else
    {
        //ID3D11Texture2D* pTexRestored = NULL; // @oooii-tony: Add an output texture (rather than saving a file)
        printf( "\tDecoding from BC6H..." );
        V_RETURN( CPU_EncodeDecode( g_pDevice, g_pContext, pSourceTexture, DXGI_FORMAT_R32G32B32A32_FLOAT, ppUncompressedTexture ) ); // @oooii-tony: Add an output texture (rather than saving a file)
        printf( "done\n" );
...
}; // @oooii-tony: end of class BC6HBC7EncoderDecoder11_Sample

// @oooii-tony: Client-facing API:
bool oD3D11EncodeBC67(const WCHAR* _EncodeType, bool _UseGPU, ID3D11Texture2D* _pSourceTexture, ID3D11Texture2D** _ppCompressedTexture)
{
	BC6HBC7EncoderDecoder11_Sample* pSample = new BC6HBC7EncoderDecoder11_Sample(_pSourceTexture);
	WCHAR args[4][_MAX_PATH];
	wcscpy_s(args[0], _MAX_PATH, L"DummyAppPath");
	wcscpy_s(args[1], _MAX_PATH, _EncodeType);
	wcscpy_s(args[2], _MAX_PATH, L"DummyFilename.png");
	wcscpy_s(args[3], _MAX_PATH, L"DummyFilename.png");
	...
	{
		argc++;
		wcscpy_s(args[2], _MAX_PATH, L"/cpu");
	}
	...
	int result = pSample->wmain(argc, ptrs, _ppCompressedTexture);
	delete pSample;
	return 0 == result;
}

bool oD3D11EncodeBC7(ID3D11Texture2D* _pSourceTexture, bool _UseGPU, ID3D11Texture2D** _ppCompressedTexture) { return oD3D11EncodeBC67(L"/bc7", _UseGPU, _pSourceTexture, _ppCompressedTexture); }
bool oD3D11EncodeBC6HS(ID3D11Texture2D* _pSourceTexture, bool _UseGPU, ID3D11Texture2D** _ppCompressedTexture) { return oD3D11EncodeBC67(L"/bc6hs", _UseGPU, _pSourceTexture, _ppCompressedTexture); }
bool oD3D11EncodeBC6HU(ID3D11Texture2D* _pSourceTexture, bool _UseGPU, ID3D11Texture2D** _ppCompressedTexture) { return oD3D11EncodeBC67(L"/bc6hu", _UseGPU, _pSourceTexture, _ppCompressedTexture); }
bool oD3D11DecodeBC6orBC7(ID3D11Texture2D* _pSourceTexture, bool _UseGPU, ID3D11Texture2D** _ppUncompressedTexture) { return oD3D11EncodeBC67(L"/decode", _UseGPU, _pSourceTexture, _ppUncompressedTexture); }
