...
HRESULT CreateDevice( ID3D11Device** ppDeviceOut, ID3D11DeviceContext** ppContextOut, BOOL bForceRef /*= FALSE*/ )
{
    ...
    *ppContextOut = NULL;
    ...
    if ( !bForceRef )
    {
        ...
        // when copy/paste this code snippet to your title, it is generally recommended to leave the fifth parameter to be NULL,
        // which will try to create greatest feature level available.
        //
        // This sample only uses CS4x shaders, there is no benefit creating a FL11 device even if the hardware is FL11 capable
        // In addition, this will cause extra test burdens if FL11 device is created.

        D3D_FEATURE_LEVEL flIn[] = { D3D_FEATURE_LEVEL_10_0 };
        ...
        hr = Dynamic_D3D11CreateDevice( NULL,                        // Use default graphics card
                                         ...
                                         flIn,                        // For the purpose of this sample, we cap the FL to 10 to reduce test burden, 
                                                                      // In practice, it is generally recommended to leave this parameter NULL (and next parameter 0),
...
again:
    ...
    } else
    {
        ...
        if ( !ReadFile( hFile, &dwVersion, sizeof(dwVersion), &dwBytesRead, NULL ) )
        ...
        if ( !ReadFile( hFile, pBlob->GetBufferPointer(), (DWORD)pBlob->GetBufferSize(), &dwBytesRead, NULL ) )
            return HRESULT_FROM_WIN32( GetLastError() );
