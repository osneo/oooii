...
#include <BC7DecodeByteCode.h> // @oooii-tony: Move shader byte code into compiled binary
#include <BC7Encode_EncodeBlockCSByteCode.h> // @oooii-tony: Move shader byte code into compiled binary
#include <BC7Encode_TryMode02CSByteCode.h> // @oooii-tony: Move shader byte code into compiled binary
#include <BC7Encode_TryMode137CSByteCode.h> // @oooii-tony: Move shader byte code into compiled binary
#include <BC7Encode_TryMode456CSByteCode.h> // @oooii-tony: Move shader byte code into compiled binary
...
HRESULT CGPUBC7Encoder::Initialize( ID3D11Device* pDevice, ID3D11DeviceContext* pContext )
{
    ...
    printf( "\tCreating Compute Shaders..." );
		...
		hr = pDevice->CreateComputeShader(BC7Encode_TryMode456CSByteCode, sizeof(BC7Encode_TryMode456CSByteCode), NULL, &m_pTryMode456CS );
		if ( FAILED( hr ) ) return hr;

		hr = pDevice->CreateComputeShader(BC7Encode_TryMode137CSByteCode, sizeof(BC7Encode_TryMode137CSByteCode), NULL, &m_pTryMode137CS );
		if ( FAILED( hr ) ) return hr;

		hr = pDevice->CreateComputeShader(BC7Encode_TryMode02CSByteCode, sizeof(BC7Encode_TryMode02CSByteCode), NULL, &m_pTryMode02CS );
		if ( FAILED( hr ) ) return hr;

		hr = pDevice->CreateComputeShader(BC7Encode_EncodeBlockCSByteCode, sizeof(BC7Encode_EncodeBlockCSByteCode), NULL, &m_pEncodeBlockCS );
		if ( FAILED( hr ) ) return hr;
...
HRESULT CGPUBC7Encoder::GPU_BC7EncodeAndSave( ID3D11Texture2D* pSourceTexture, DXGI_FORMAT fmtEncode, WCHAR* strDstFilename, ID3D11Texture2D** ppOutCompressedTexture ) // @oooii-tony: Don't write to file, just pass out the texture
{
    HRESULT hr = S_OK;
    ...
    //wprintf( L"\tSaving to %s...", strDstFilename ); // @oooii-tony: Squelch spew
    V_RETURN( GPU_SaveToFile( m_pDevice, m_pContext,
                              pSourceTexture, strDstFilename,
                              fmtEncode, m_pEncodedTextureAsBuf, ppOutCompressedTexture ) ); // @oooii-tony: Don't write to file, just pass out the texture
    printf( "done\n" );
...
HRESULT CGPUBC7Encoder::GPU_SaveToFile( ID3D11Device* pDevice, ID3D11DeviceContext* pContext,
                                        ...
                                        WCHAR* strFilename,
                                        DXGI_FORMAT dstFormat, ID3D11Buffer* pTextureAsBuf, ID3D11Texture2D** ppOutCompressedTexture ) // @oooii-tony: Don't write to file, just pass out the texture
{
    HRESULT hr = S_OK;
    ...
    //hr = D3DX11SaveTextureToFile( pContextFL11, pDstTexture, D3DX11_IFF_DDS, strFilename ); // @oooii-tony: Don't write to file, just pass out the texture
		(*ppOutCompressedTexture) = pDstTexture; // @oooii-tony
		(*ppOutCompressedTexture)->AddRef(); // @oooii-tony
		hr = S_OK; // @oooii-tony
                                       ...
                                       DXGI_FORMAT dstFormat, ID3D11Buffer** ppDstTextureAsBufOut )
{
    ...
    while (num_blocks > 0)
    {
        int n = __min(num_blocks, MAX_BLOCK_BATCH); // @oooii-tony: min -> __min because common prop sheets define NOMINMAX
...
HRESULT CGPUBC7Decoder::Initialize( ID3D11Device* pDevice, ID3D11DeviceContext* pContext )
{
    ...
    printf( "\tCreating Compute Shaders..." );
		...
		hr = pDevice->CreateComputeShader(BC7DecodeByteCode, sizeof(BC7DecodeByteCode), NULL, &m_pCS );
		if ( FAILED( hr ) ) return hr;
...
HRESULT CGPUBC7Decoder::GPU_BC7DecodeAndSave( ID3D11Texture2D* pSourceTexture, WCHAR* strDstFilename, ID3D11Texture2D** ppOutCompressedTexture ) // @oooii-tony: Add an output texture (rather than saving a file)
{
    HRESULT hr = S_OK;
    ...
    pSrcTexDevice->GetImmediateContext( &pSrcTexContext ); pSrcTexContext->Release();
    {
        ...
        sbInputDesc.ByteWidth = texSrcDesc.Height * texSrcDesc.Width * sizeof( BufferBC7 ) / BLOCK_SIZE;
    ...
    hr = GPU_BC7DecodeAndSaveInternal( pBufferInput, texSrcDesc.Width, texSrcDesc.Height, texSrcDesc.Format, strDstFilename, ppOutCompressedTexture ); // @oooii-tony: Add an output texture (rather than saving a file)
...
HRESULT CGPUBC7Decoder::GPU_BC7DecodeAndSaveInternal( ID3D11Buffer* pSrcTextureAsBuf, UINT uWidth, UINT uHeight, DXGI_FORMAT fmtSrc, WCHAR* strDstFilename, ID3D11Texture2D** ppOutCompressedTexture ) // @oooii-tony: Add an output texture (rather than saving a file)
{
    ID3D11ShaderResourceView* pSRV = NULL;
    ...
    D3D11_BUFFER_DESC sbOutDesc;
    {
        ...
        sbOutDesc.ByteWidth = uWidth * uHeight * sizeof( BufferR8G8B8A8 );
    ...
    //if ( strDstFilename ) // @oooii-tony: Add an output texture (rather than saving a file)
        //hr = D3DX11SaveTextureToFile( m_pContext, m_pRestoredTexture, D3DX11_IFF_DDS, strDstFilename ); // @oooii-tony: Don't write to file, just pass out the texture
		(*ppOutCompressedTexture) = m_pRestoredTexture;
		(*ppOutCompressedTexture)->AddRef();
