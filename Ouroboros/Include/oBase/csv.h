...
public:
	...
	csv(const char* _URI, char* _pData, deallocate_fn _Delete, size_t _EstNumRows = 100, size_t _EstNumCols = 20)
		...
		, NumCols(0)
	{
		...
		for (const auto& cell : Cells)
			Size += cell.capacity() * sizeof(index_type);
	}

	csv(const char* _URI, const char* _pData, const allocator& alloc, size_t _EstNumRows = 100, size_t _EstNumCols = 20)
		: Buffer(_URI, _pData, alloc, "csv doc")
		, NumCols(0)
	{
		Size = sizeof(*this) + strlen(Buffer.data) + 1;
		Cells.reserve(_EstNumRows);
	...
	inline const char* cell(size_t _ColIndex, size_t _RowIndex) const { return (_RowIndex < Cells.size() && _ColIndex < Cells[_RowIndex].size()) ? Buffer.data + Cells[_RowIndex][_ColIndex] : ""; }
